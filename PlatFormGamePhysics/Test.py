    ...
    for index in range(len(dynamicList)):
        ...
        if len(dynamicList)>1:
            for i in range(index+1, len(dynamicList)):
                if rectangleOverlapX(object1.pos, dynamicList[i].pos):
                    pair.append([object1, dynamicList[i], "x"])
                    pairs+=1
                if rectangleOverlapY(object1.pos, dynamicList[i].pos):
                    pair.append([object1, dynamicList[i], "y"])
                    pairs +=1
        for i in staticList:
            if rectangleOverlapX(object1.pos, i.pos):
                pair.append([object1, i, "x"])
                pairs+=1
            if rectangleOverlapY(object1.pos, i.pos):
                pair.append([object1, i, "y"])
                pairs+=1
    update(dynamicList, staticList, 0, pair, pairs)
    while Frame<1:
        pair = []
        pairs = 0
        TIME = 1
