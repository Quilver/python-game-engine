...
class Block():
    def __init__(self, x, y, width=50, length=15, colour="brown", moving=False):
        self.canvas=canvas
        self.x = x
        self.y = y
        self.moving = moving
        self.mass = width * length
        self.id = canvas.create_rectangle(self.x, self.y, self.x+width, self.y+length,fill=colour)
        self.pos = self.canvas.coords(self.id)
        if moving:
            MOVING_OBJECTS.append(self)
            self.velocity=10
            #add to a dynamic list
        else:
            WALL.append(self.pos)

    def draw(self, static, player):
        self.player = player
        self.static = static
        if self.moving:
            self.chase(player.pos)
